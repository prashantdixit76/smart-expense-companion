...
const AddIncome = () => {
  ...
  const fetchIncomes = async () => {
    if (!user) return;
    const { data } = await supabase.from('incomes').select('*').eq('user_id', user.id).order('date', { ascending: false });
    setIncomes(data || []);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await supabase.from('incomes').insert({ user_id: user.id, amount, source: form.source, description: form.description, date: form.date });
  ...
  const handleUpdate = async () => {
    ...
    await supabase.from('incomes').update({ amount, source: editForm.source, description: editForm.description, date: editForm.date }).eq('id', editItem.id);
